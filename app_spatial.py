...
with st.expander("⚙️ Configuration - WBS Level & Normalization Matrix", expanded=True):
    ...
    st.caption("Vous pouvez définir un nom commun et des commentaires pour le groupe, ainsi qu'une complexité **par système**. Les colonnes des systèmes non concernés sont ignorées.")
    ...
    if not df_natural_edit.empty:
        ...
        for sys in files_list:
            ...
            column_config[col_name] = st.column_config.NumberColumn(
                ...
                step=0.1,
                format="%.1f",
                help=f"Facteur de normalisation pour {sys} (défaut 1.0). Laissez vide si non applicable."
            )
...
# --- ONGLET 4 : Bridges (avec heatmap en permanence) ---
with tabs[4]:
    if 'pivot_norm_common' not in st.session_state:
        ...
        if base_n != target_n:
            # Graphique Bridge pleine largeur
            st.plotly_chart(draw_bridge(pivot_norm_common, base_n, target_n, "Normalized "),
                            use_container_width=True, key="norm_bridge")

            # --- Heatmap des variations annuelles pour cette paire (affichée directement) ---
            st.markdown("---")
            st.subheader(f"Heatmap des variations annuelles moyennes ({base_n} → {target_n})")
            st.markdown("""
            Cette heatmap montre la **variation annuelle moyenne** (en %) des coûts normalisés entre les deux devis sélectionnés.
            - **Rouge** : hausse (> 0%)
            - **Bleu** : baisse (< 0%)
            - L'intensité est proportionnelle à l'ampleur.
            Les lots sans données sur l'un des deux devis sont ignorés.
            """)

            # Calcul de la durée entre les deux devis
            date_base = dates[base_n]
            date_target = dates[target_n]
            delta_years = (date_target - date_base).days / 365.25
            if delta_years <= 0:
                st.warning("La durée entre les deux devis est nulle ou négative.")
            else:
                # Récupérer les coûts normalisés par Common Name pour les deux devis
                df_common_local = st.session_state.df_common
                df_pair = df_common_local[df_common_local['System'].isin([base_n, target_n])]
                pivot_pair = df_pair.pivot_table(index='Common_Name', columns='System', values='Normalized_Cost', aggfunc='sum').fillna(0)
                ...
                for common in pivot_pair.index:
                    ...
                    if c_base != 0 and not np.isnan(c_base) and not np.isnan(c_target):
                        ...
                        heat_data.append({
                            'Common Name': common,
                            'Variation annuelle (%)': round(var_ann, 1)
                        })

                if heat_data:
                    df_heat = pd.DataFrame(heat_data).set_index('Common Name')
                    df_heat = df_heat.sort_values('Variation annuelle (%)', ascending=False)
                    ...
                    fig_heat.update_layout(
                        title=f"Variation annuelle moyenne ({base_n} → {target_n})",
                        xaxis_title="Période",
                        yaxis_title="Lot",
                    ...
                    st.info("Aucun lot commun avec des coûts non nuls pour calculer une variation.")
        else:
...
# --- ONGLET 6 : competitiveness deep dive (avec pastilles) ---
with tabs[6]:
    if 'df_common' not in st.session_state:
        st.warning("Please apply the configuration first.")
    else:
...
* **Bridges**: Visual decomposition of cost differences between any two bids, grouped by Common Name, avec heatmap des variations annuelles.
