...
# --- ONGLET 1 : WP SUMMARY (mapping et données agrégées) ---
with tabs[0]:
    st.divider()
...
# --- GRAPHIQUE À BARRES (par WBS_2) ensuite ---
    ...
    if os.path.exists(MAPPING_FILE):
        ...
        if not all(col in df_mapping_all.columns for col in required_cols):
            st.warning("Le fichier de mapping est corrompu ou d'une version antérieure. Réinitialisation.")
...
# --- ONGLET 2 : SNAPSHOTS (sélection par checkboxes simplifiée) ---
with tabs[1]:
    if 'df_global' not in st.session_state:
        st.warning("Veuillez d'abord configurer le niveau et le mapping dans l'onglet WP Summary.")
    else:
        ...
        # Affichage simple en 5 colonnes sans cadre personnalisé
        # (mais on peut ajouter une bordure avec un expander si souhaité)
        st.markdown("---")
        cols_per_row = 5
...
st.markdown("""
* **WP Summary**: Choose WBS level, view a global overview of Level 1 costs, edit mapping, and view aggregated data per Work Package.
* **Snapshots**: Raw and normalized cost views, with bridge charts. Select WPs via checkboxes (Select All/Clear All).
* **Drifts**: Global trend and per-WP annualized drift (calculated across all systems). No system column, just the Work Package.
